...
var skillWheel = mainContent.append('div').style('width', width + 'px').style('height', height + 'px').style('margin-left', '5%').style('margin-right', '5%').style('margin-top', '0px').style('float', 'right').attr('id', 'skillWheel').style('display', 'none');
skillWheel.remove();
...
function makeGrid(){
	...
	for (btnTypeIdx = 0; btnTypeIdx < btnTypes.length; btnTypeIdx++)
	{
		...
		btnTitle[btnTypeIdx] = buttons[btnTypeIdx].append('div').style('height', btnTitleHeight + 'px').attr('class', 'nameHeader').style('color', baseHcl.toString()).style('margin-right', '25px').style('margin', '10px').style('padding', '7px').text(btnTypes[btnTypeIdx]); // + "<br>");
		btnContent[btnTypeIdx] = buttons[btnTypeIdx].append('div').style('height', btnContentHeight + 'px').attr('id', 'btnContent' + btnTypeIdx);
		if ( (x < 600) || (y < 300) )
		{
...
function buttonClicked(id) {
	...
	{	
		...
		if ( btnTypes[id] == 'Skills')
		{
			...
			appContent.insert('#skillWheel', ':first-child');
		}
		/*
