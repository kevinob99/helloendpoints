...
google.devrel.samples.hello.CLIENT_ID =
    '262342788785-bieclaai8dq9jeehgcddjglq9dti9uvf.apps.googleusercontent.com';
...
mainContent.append('div').text('Get Greeting').attr('id', 'getGreeting').style('width', '100px').style('float', 'left');
mainContent.append('div').text('List Greeting').attr('id', 'listGreeting').style('width', '100px').style('float', 'left');
mainContent.append('div').text('Multiply Greetings').attr('id', 'multiplyGreetings').style('width', '100px').style('float', 'left');
mainContent.append('div').text('Authed Greeting').attr('id', 'authedGreeting').style('width', '100px').style('float', 'left');
mainContent.append('div').text('Sign In Button').attr('id', 'signinButton').style('width', '100px').style('float', 'left');
mainContent.append('div').attr('id', 'outputLog').style('float', 'left');
	...
	while ((hexId < appTypes.length ) && (loopCounter++ < 500))
	{
		...
		while (centx < (right + radius)) {
			...
			if ((hexagon.attr('outOfBounds') == 'false') && (hexId < appTypes.length) ) //numApps) )
			{
				...
				if (hexId == 1)
				{
					hexagon.attr("xlink:href", "https://psychic-fold-618.appspot.com/Profile")
				}
...
function hex2AppAnimation(id, appTop, appLeft, appWidth, appHeight){
	...
	*/
	...
	transitionHexSVG = /*transitionHexDiv*/mainContent.append('svg'); //mainContent.append('svg');
